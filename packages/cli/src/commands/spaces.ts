...
export class SpacesCommand extends BaseCommand {
      ...
      .action(async (options) => {
        ...
        try {
          ...
          response.data.forEach((s) => {
            ...
            console.log(
              `  ${chalk.yellow(s.id)}  ${chalk.bold(s.name)}  ${chalk.dim(s.slug)}${state}`
            )
          })
        } catch (e) {
          this.handleError(e)
    ...
    members
      ...
      .action(async (spaceId, options) => {
        ...
        try {
          ...
          res.data.forEach((m) => {
            console.log(
              `  ${chalk.yellow(m.id)}  ${chalk.bold(m.name ?? '')}  ${chalk.dim(m.email ?? '')}  ${m.role ?? ''}`
            )
          })
        } catch (e) {
          this.handleError(e)
    ...
    backups
      ...
      .action(async (spaceId, options) => {
        ...
        try {
          ...
          res.data.forEach((b) =>
            console.log(
              `  ${chalk.yellow(b.id)}  ${b.name ?? ''}  ${chalk.dim(b.created_at ?? '')}`
            )
          )
        } catch (e) {
          this.handleError(e)
          ...
          const { confirm } = await inquirer.prompt([
            {
              type: 'confirm',
              name: 'confirm',
              message: `Delete backup ${chalk.yellow(backupId)}?`,
              default: false,
            },
          ])
          if (!confirm) return console.log('Aborted')
    ...
    mig
      ...
      .action(async (spaceId, options) => {
        ...
        try {
          ...
          res.data.forEach((m) =>
            console.log(`  ${chalk.yellow(m.id)}  ${chalk.dim(m.created_at ?? '')}`)
          )
        } catch (e) {
          this.handleError(e)
        }
          ...
          const { confirm } = await inquirer.prompt([
            {
              type: 'confirm',
              name: 'confirm',
              message: `Delete migration ${chalk.yellow(migrationId)}?`,
              default: false,
            },
          ])
          if (!confirm) return console.log('Aborted')
          ...
          const { confirm } = await inquirer.prompt([
            {
              type: 'confirm',
              name: 'confirm',
              message: `Cancel subscription for space ${chalk.yellow(spaceId)}?`,
              default: false,
            },
          ])
          if (!confirm) return console.log('Aborted')
    ...
    ns.command('audit-logs')
      ...
      .action(async (spaceId, options) => {
        ...
        try {
          ...
          if (options.perPage) params.per_page = Number(options.perPage)
          if (options.page) params.page = Number(options.page)
          const res = await this.client.spaces.getAuditLogs(
            spaceId,
          ...
          res.data.forEach((entry) => {
            console.log(
              `  ${chalk.dim(entry.created_at ?? '')}  ${chalk.bold(entry.operation)}  ${entry.owner_name ?? entry.owner_id ?? ''}`
            )
          })
        } catch (e) {
          this.handleError(e)
          ...
          const { confirm } = await inquirer.prompt([
            {
              type: 'confirm',
              name: 'confirm',
              message: `Delete AI config ${chalk.yellow(configId)}?`,
              default: false,
            },
          ])
          if (!confirm) return console.log('Aborted')
  ...
  private async promptCreateSpace(opts: SpaceCommandOptions): Promise<CreateSpaceParams> {
    const answers = await inquirer.prompt([
      ...
      },
      {
        type: 'input',
        name: 'slug',
        message: 'Slug (lowercase, hyphens):',
        default: opts.slug,
        validate: (v) =>
          /^[a-z0-9-]+$/.test(v) ? true : 'Only lowercase letters, numbers, hyphens',
      },
      {
        type: 'input',
        name: 'team_id',
    ...
    const answers = await inquirer.prompt([
      {
        type: 'input',
        name: 'name',
        message: 'New name (leave blank to keep):',
        default: opts.name || '',
      },
      {
        type: 'input',
        name: 'slug',
        message: 'New slug (leave blank to keep):',
        default: opts.slug || '',
      },
      {
        type: 'input',
  ...
  private printHierarchy(teams: Team[], spaces: Space[], indent = 0): void {
    if (indent === 0) {
      ...
      orphanSpaces.forEach((s) => {
        console.log(
          `${'  '.repeat(indent + 1)}${chalk.cyan('◆')} ${chalk.bold(s.name)} ${chalk.dim(s.id)}`
        )
      })
  ...
  private printTeamNode(team: Team, allTeams: Team[], allSpaces: Space[], indent: number): void {
    console.log(
      `${'  '.repeat(indent)}${chalk.blue('▸')} ${chalk.bold(team.name)} ${chalk.dim(team.id)}`
    )
