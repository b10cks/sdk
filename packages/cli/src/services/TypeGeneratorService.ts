...
export class TypesGeneratorService extends BaseService {
  ...
  private generateTypesContent(blocks: Block[]): string {
    let content = `// This file is auto-generated by \`b10cks generate types\`. Do not edit manually.
...
`
  ...
  private getInterfaceName(slug: string): string {
    return `B10cks${this.toPascalCase(slug)}`
  }
    ...
    switch (this.normalizeSchemaType(schemaType)) {
      ...
      case 'blocks': {
        ...
        if (
          ...
          const uniqueTypes = [...new Set(whitelistTypes)]
          if (uniqueTypes.length === 1) return `${uniqueTypes[0]}[]`
          if (uniqueTypes.length > 1) return `Array<${uniqueTypes.join(' | ')}>`
        }
